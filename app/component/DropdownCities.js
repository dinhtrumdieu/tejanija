...
export default class DropdownCities extends React.Component {
    state = {
        isVisible: false,
    ...
    show(items, options, keyObject = 'province_name') {
        const newItems = items.map((item, index) => ({label: item, key: `${index}`}));
    ...
    _renderItem = ({item, index}) => {
        const {options, keyObject} = this.state;
        const {
