...
import {
    ...
    Text,
    TouchableOpacity,
    SectionList, ActivityIndicator, Platform,
} from 'react-native';
    ...
    render() {
        ...
        return (
            <View style={[styles.container]}>
                {
                    (this.props.data.length === 0 && !this.props.isLoaded) &&
                    <ActivityIndicator style={{marginVertical: scale(40)}} animating size={size}
                                       color={CommonColors.mainColor}/>
                }
                {this.props.data.length === 0 && this.props.isLoaded && <EmptyView/>}
                <SectionList
                    ...
                    renderSectionHeader={({section: {_id}}) => (
                        <Text style={Platform.OS === 'ios' ? styles.headerTitleIOS : styles.headerTitleAndroid}>
                            {_id.day} / {_id.month}</Text>
                    )}
                    {...this.props.options}
                />
            </View>
    ...
    _renderCircle(rowData, rowID) {
        ...
        switch (this.props.innerCircle) {
            case 'icon':
                ...
                if (rowData.icon) {
                    iconSource = rowData.icon.constructor === String ? {uri: rowData.icon} : rowData.icon;
                }
                let iconStyle = {
                    height: circleSize,
                    width: circleSize,
